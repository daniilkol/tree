...
int fap (Node* root,int &n,int &m)
{

    if(root->goto_left(root)==NULL&&root->goto_right(root)==NULL)
    {
        if (m>n)
            n=m;
    }
    ...
    {
        m++;
        if(root->goto_right(root)!=NULL&&root->goto_left(root)!=NULL)
            fap(root->goto_right(root),n,m);
            else if(root->goto_right(root)!=NULL)
            fap(root->goto_right(root),n,m);
            else fap(root->goto_left(root),n,m);
